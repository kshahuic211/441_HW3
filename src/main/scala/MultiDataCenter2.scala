import MultiDataCenter1.{BROKER_NETWORK_LATENCY, DC_NETWORK_BANDWIDTH}
...
// This program simulates multiple datacenters distributed across different time zones connected using Tree Topology
// Vms are allocated based on Round-Robin policy
// The hosts are connected to each other in the datacenters using Edge switches

object MultiDataCenter2 extends App {
  /**
  ...
  val logger = CreateLogger(classOf[MultiDataCenter2.type])

  val INIT_UTILIZATION_RATIO = applicationConf.getDouble("DataCenter2.initUtilizationRatio")
  val MAX_UTILIZATION_RATIO = applicationConf.getDouble("DataCenter2.maxUtilizationRatio")
  val SCHEDULING_INTERVAL = applicationConf.getDouble("DataCenter2.schedulingInterval")

  val ARCH = applicationConf.getString("DataCenter2.arch")
  val OS = applicationConf.getString("DataCenter2.os")
  val VMM = applicationConf.getString("DataCenter2.vmm")


  val SIMULATION_TIME = applicationConf.getInt("DataCenter2.simulationTime")
  val TIME_ZONES = applicationConf.getDoubleList("DataCenter2.timeZones")

  val BROKER_NETWORK_LATENCY = applicationConf.getInt("DataCenter2.brokerNetworkLatency")
  val DC_NETWORK_BANDWIDTH = applicationConf.getInt("DataCenter2.dcNetworkBandwidth")
  val DC_INTERNAL_NETWORK_LATENCY = applicationConf.getInt("DataCenter2.dcInternalNetworkLatency")
  val HOST_COUNT = applicationConf.getInt("DataCenter2.hostCount")
  val VMs_PER_HOST = applicationConf.getInt("DataCenter2.vmsPerHost")
  ...
  val CLOUDLETS = applicationConf.getInt("DataCenter2.cloudLets")
  val CLOUD_LEN = applicationConf.getInt("DataCenter2.cloudLet.length")
  val CLOUD_PE = applicationConf.getInt("DataCenter2.cloudLet.pe")

  val CPU_COST = applicationConf.getDouble("DataCenter2.cost.cpu")
  val MEM_COST = applicationConf.getDouble("DataCenter2.cost.mem")
  val STORAGE_COST = applicationConf.getDouble("DataCenter2.cost.storage")
  val BW_COST = applicationConf.getDouble("DataCenter2.cost.bw")

  // calls startSimulation()
  startSimulation();

  def startSimulation(): Unit = {
    logger.info("The Simulation of MultiDataCenter2 has started")

    val simulation = new CloudSim()

    val broker0 = createBroker(simulation)
    logger.info("The broker for MultiDataCenter2 has been created")
    ...
    printCosts(broker0)
    logger.info("The Simulation of MultiDataCenter2 has ended")
  }
  ...
  def createDatacenters(simulation: CloudSim): List[NetworkDatacenter] = {
    val datacenterList = TIME_ZONES.map(timeZone => {
      ...
      val networkDatacenter = new NetworkDatacenter(simulation, hostList.asJava, new VmAllocationPolicyRoundRobin);
  ...
  // creates the Tree Network Topology
  def configureNetwork(simulation: CloudSim, datacenterList: List[NetworkDatacenter], broker0: DatacenterBroker): Unit = {
    ...
    networkTopology.addLink(datacenterList(1), datacenterList(3), DC_NETWORK_BANDWIDTH, DC_INTERNAL_NETWORK_LATENCY)
  }
  ...
  def isVmOverloaded(vm: Vm) = {
    vm.getCpuPercentUtilization > 0.75
  }
